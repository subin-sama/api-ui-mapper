...
// Per-generation map of fieldName -> { str, num, len }: the values the app's own
// code checks the field against (so a mock for `responseCode` becomes the "00"
// the code checks, not a generic guess). Set at the start of each generation.
let activeHints = {};

// Happy-path bias for literal-union members (e.g. 'pending' | 'completed' | 'failed').
...
const ERROR_TOKENS = new Set([
  ...
  'pending', 'inactive', '99', '98', '90', '401', '403', '404', '500'
]);

// Pick the happy-path member of a union: the value the code checks for, else a
// success-token literal, else a non-error member, else the first.
function chooseUnionMember(members, key) {
  const lit = (mem) => { const x = mem.trim(); return /^['"`]/.test(x) ? x.replace(/['"`]/g, '') : null; };
  const hint = key ? activeHints[key] : null;
  if (hint) {
    for (const mem of members) { const v = lit(mem); if (v != null && (v === hint.str || v === hint.num)) return mem; }
  }
  for (const mem of members) { const v = lit(mem); if (v != null && SUCCESS_TOKENS.has(v.toLowerCase())) return mem; }
  const nonErr = members.filter(mem => { const v = lit(mem); return !(v != null && ERROR_TOKENS.has(v.toLowerCase())); });
  return nonErr[0] || members[0];
}
...
function mockPrimitive(type, key) {
  ...
  if (hint) {
    if ((t === 'number' || t === 'bigint') && hint.num !== undefined) {
      ...
      if (hint.str !== undefined) return hint.str;
      // The code checks this string's length (e.g. otp.length === 6) -> match it.
      if (hint.len !== undefined) {
        const n = parseInt(hint.len, 10);
        if (n > 0 && n <= 64) { let s = ''; for (let i = 0; i < n; i++) s += String((i + 1) % 10); return s; }
      }
    }
  }
...
function mockForType(annotation, typeDefs, key, depth, path) {
  ...
  // Union -> pick the happy-path member (code-checked value / success literal)
  const union = splitTopLevel(t, ['|']).filter(p => p && p !== 'undefined' && p !== 'null');
  if (union.length > 1) return mockForType(chooseUnionMember(union, key), typeDefs, key, depth, path);
