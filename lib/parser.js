...
function buildRouteRegistry(srcDir) {
  ...
  files.forEach(file => {
    ...
    const initM = /\binitialRouteName=["']([^"']+)["']/.exec(content);
    const importMap = {};
    ...
    navInfo[file] = { routes, initialRoute: initM ? initM[1] : null, importMap };
  });
...
function runParser(options) {
    ...
    files.forEach(file => {
      ...
      exportsInFile.forEach((exp, idx) => {
        ...
        apiFunctions[exp.name] = {
          ...
          mockReqJson: reqType ? generateMockFromType(reqType, typeDefinitions) : null,
          mockResJson: resType ? generateMockFromType(resType, typeDefinitions) : null
        };
      });
    });
  });
  ...
  const routeResolution = buildRouteRegistry(srcDir);
