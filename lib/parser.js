...
// Tokens that read as a failure — never used as a happy-path mock value.
const ERROR_TOKENS = new Set([
  'error', 'fail', 'failed', 'failure', 'invalid', 'denied', 'deny', 'reject', 'rejected',
  'unauthorized', 'forbidden', 'notfound', 'timeout', 'expired', 'blocked', 'cancelled', 'cancel',
  '99', '98', '90', '401', '403', '404', '500', '501', '502', '503'
]);

// Scan a `switch (x.field) { case 'v': ... }` block (brace-aware) for its cases.
function forEachSwitchCase(code, cb) {
  const re = /switch\s*\(\s*[\w.$[\]]*?\.([A-Za-z_$][\w$]*)\s*\)\s*\{/g;
  let m;
  while ((m = re.exec(code)) !== null) {
    const field = m[1];
    ...
    const caseRe = /case\s+(?:(['"])([^'"]+)\1|(-?\d+(?:\.\d+)?))\s*:/g;
    let c;
    const block = code.slice(start, i);
    while ((c = caseRe.exec(block)) !== null) {
      if (c[2] !== undefined) cb('str', field, c[2]);
      else cb('num', field, c[3]);
    }
  }
}

// Scan the codebase for the values fields are checked against — equality
// (`res.responseCode === '00'`), switch/case, and length (`otp.length === 6`)
// — so mocks use what the app actually expects on the happy path rather than a
// generic guess. Returns fieldName -> { str, num, len }, success-biased and
// avoiding known error values.
function collectFieldHints(srcDir) {
  ...
  const lenRe = /\.([A-Za-z_$][\w$]*)\.length\s*[!=<>]==?\s*(\d+)/g;                   // .field.length === 6
  getFiles(srcDir).forEach(f => {
    let code; try { code = fs.readFileSync(f, 'utf8'); } catch (e) { return; }
    let m;
    strRe.lastIndex = strRevRe.lastIndex = numRe.lastIndex = lenRe.lastIndex = 0;
    ...
    while ((m = lenRe.exec(code)) !== null) bump(len, m[1], m[2]);
    forEachSwitchCase(code, (kind, field, value) => bump(kind === 'num' ? num : str, field, value));
  });
  const pick = (bag) => {
    const out = {};
    for (const field in bag) {
      ...
      if (succ.length) { succ.sort((a, b) => b[1] - a[1]); out[field] = succ[0][0]; continue; }
      const nonErr = entries.filter(([v]) => !ERROR_TOKENS.has(String(v).toLowerCase()));
      if (!nonErr.length) continue; // only error values -> no hint, fall back to default
      nonErr.sort((a, b) => b[1] - a[1]);
      out[field] = nonErr[0][0];
    }
    return out;
  };
  const bestStr = pick(str), bestNum = pick(num), bestLen = pick(len);
  const hints = {};
  new Set([...Object.keys(bestStr), ...Object.keys(bestNum), ...Object.keys(bestLen)]).forEach(f => {
    hints[f] = { str: bestStr[f], num: bestNum[f], len: bestLen[f] };
  });
  return hints;
}
